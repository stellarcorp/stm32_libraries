...
static uint32_t WavProcess_ParseHeader(uint8_t* pHeader, WAVE_FormatTypeDef* pAudioFormatDataStruct)
{
  ...
  Temp = ReadUnit(4, pdBIG_ENDIAN, pHeader, HeadTabIdx);
  if (Temp != CHUNK_ID)
  {
  ...
  pAudioFormatDataStruct->RIFFchunksize = ReadUnit(4, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  HeadTabIdx += 4;
  ...
  Temp = ReadUnit(4, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  if (Temp != 0x10)
  {
    ExtraFormatBytes = 1;
  }
  ...
  pAudioFormatDataStruct->FormatTag = ReadUnit(2, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  ...
  pAudioFormatDataStruct->NumChannels = ReadUnit(2, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  HeadTabIdx += 2;

  /* Read the Sample Rate ----------------------------------------------------*/
  pAudioFormatDataStruct->SampleRate = ReadUnit(4, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  HeadTabIdx += 4;

  /* Read the Byte Rate ------------------------------------------------------*/
  pAudioFormatDataStruct->ByteRate = ReadUnit(4, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  HeadTabIdx += 4;

  /* Read the block alignment ------------------------------------------------*/
  pAudioFormatDataStruct->BlockAlign = ReadUnit(2, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  HeadTabIdx += 2;

  /* Read the number of bits per sample --------------------------------------*/
  pAudioFormatDataStruct->BitsPerSample = ReadUnit(2, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  HeadTabIdx += 2;
  ...
  while (ReadUnit(4, pdBIG_ENDIAN, pHeader, HeadTabIdx) != DATA_ID)
  {
  ...
  pAudioFormatDataStruct->AudioLength = ReadUnit(4, pdLITTLE_ENDIAN, pHeader, HeadTabIdx);
  HeadTabIdx += 4;
...
static uint32_t ReadUnit(uint8_t NbrOfBytes, uint32_t BytesFormat, uint8_t* pBuff, uint32_t BuffIdx)
{
  ...
  if (BytesFormat == pdLITTLE_ENDIAN)
  {
